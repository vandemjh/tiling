...
function createRGBArray($url) {
  ...
  switch ($urlArray[count($urlArray)-1]) {
    ...
    case 'bmp': $image = imagecreatefrombmp($url); break;
    case 'xbm': $image = imagecreatefromxbm($url); break;
    default:
      $imageTry = imagepng(imagecreatefromstring(file_get_contents($url)), "temp");
      if ($imageTry != NULL && $imageTry != false) $image = imagecreatefrompng("temp");
      break; //not supported
    //default: break;
}
if ($image == NULL) {
